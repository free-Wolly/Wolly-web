...
const Slider = ({ messages }: any) => {
  const baseVelocity: number = 15

  const renderImages = useMemo(() => {
    return images.map((image: StaticImageData, id: number) => {
      return <Parallax key={id} image={image} baseVelocity={baseVelocity} id={id} qty={images.length} />
    })
  }, [baseVelocity])
  ...
  return (
    <div id="team" className="mx-auto w-full">
      ...
      <motion.div className="whitespace-nowrap flex flex-nowrap ">
        <motion.div
          ref={(ref) => ref?.scrollTo(ref?.scrollWidth / 2 - ref.clientWidth / 2, 0)}
